...
constructor(private val extension: KotlinQAExtension) : DefaultTask() {
    ...
    fun generateDetektConfigurationFile() {
        val configuration = Thread.currentThread().contextClassLoader
            .getResource(DETEKT_CONFIG_FILE)
            ?.readText()
            ?: error("Unable to read $DETEKT_CONFIG_FILE from the classpath, is this a bug in the Kotlin QA plugin?")
